...
class Book extends Component {
  ...
  displayIngredients(recipe) {
    var htmlSt =
      '<div><p style={{font-weight: "bold"; text-align: "center"}}>Ingredients</p><ul style={{text-align: "right"}}>';
    var ing = recipe.recipe.extendedIngredients.map(ingredient => {
      console.log(ingredient);
      htmlSt = htmlSt.concat(`<li>${ingredient.originalString}</li>`);
    });

    htmlSt = htmlSt.concat(
      '</ol></div><hr style={{border: 0; height: "1px"; background-image: "linear-gradient(to right, rgba(0, 0, 0, 0), rgba(0, 0, 0, 0.75), rgba(0, 0, 0, 0))"}}/>'
    );
    console.log(htmlSt);
    swal({
      title: recipe.name,
  ...
  displayInstructions(recipe) {
    var htmlStr =
      '<div><p style={{font-weight: "bold"; text-align: "center"}}>Instructions</p><ul style={{text-align: "right"}}>';
  ...
  render() {
    return (
      <div className="RBcontainer">
        ...
        <Link to="/new-recipe">
          <Button id="createARecipe" bsStyle="info">
            Create a Recipe
          </Button>
        </Link>

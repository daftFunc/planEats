...
class Schedule extends Component {
  ...
  populateSavedMeals() {
    this.state.meals.forEach((meal, i) =>
      this.state.mealNames.push(
        <option name={meal.name} key={i}>
          {meal.name}
        </option>
      )
    );
  ...
  render() {
    return (
      <div className="schedule">
        <h4>
          {moment(this.props.location.state.date).format("MMMM Do[,] YYYY")}
        </h4>
        {/*after MVP, implement feature to select a date range here*/}
        <div className="saved">
          <select id="selectedMeal">{this.state.mealNames}</select>
          <br />

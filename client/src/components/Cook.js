...
export default class Cook extends Component {
  ...
  componentDidMount() {
    Axios.get("/api/events", {
      headers: { username: JSON.parse(localStorage.profile).email }
    })
      .then(events => {
      ...
      .then(mealRecipe => {
        var instructions = mealRecipe.data[0][0].Recipes.map(recipeData => {
          ...
          return [
            ...
            recipeData.recipe.extendedIngredients
          ];
        });
  ...
  getNextEvent(events) {
    return events.reduce((a, b) => {
      ...
      var val;
      if (
        moment(momentNext).diff(moment()) > moment(momentPrev).diff(moment())
      ) {
        val = a;
      } else {
        val = b;
  ...
  <ul>
    ...
    {instructions.map((element, i) => (
      <Panel id="instruction-box">
        <p>
          <li key={element[0]}>
            <div id="title" style={{ fontWeight: "bold", fontSize: "1.5em" }}>
              {element[0]}
            </div>

...
describe("Database Queries", function() {
  before(function(done) {
    db.Recipe.sync({ force: true })
      .then(db.Users.sync({ force: true }))
      .then(() =>
        db.Users.belongsToMany(db.Recipe, {
          through: db.UsersRecipes,
          foreignkey: "MealID"
        })
      )
      .then(() =>
        db.Recipe.belongsToMany(db.Users, {
          through: db.UsersRecipes,
          foreignkey: "UserId"
        })
      )
      ...
      .then(() =>
        db.Meals.belongsToMany(db.Users, {
          through: db.UsersMeals,
          foreignkey: "MealId"
        })
      )
      .then(() =>
        db.Users.belongsToMany(db.Meals, {
          through: db.UsersMeals,
          foreignkey: "UserId"
        })
      )
      .then(() =>
        db.Recipe.belongsToMany(db.Meals, {
          through: db.MealsRecipes,
          foreignkey: "RecipeId"
        })
      )
      .then(() =>
        db.Meals.belongsToMany(db.Recipe, {
          through: db.MealsRecipes,
          foreignkey: "MealId"
        })
      )
  ...
  var recipe = [
    {
      name: "Hot Chocolate",
      ingredients:
        "1) 3 bars of milk chocolate 2) 1 cup of hot water 3) 3 tps of marshmellow"
    },
    {
  ...
  describe("Recipes", () => {
    it("should add recipe", done => {
      controller
        .addRecipe(recipeName[0], recipe[0])
    ...
    it("should retrieve all recipes for a user", done => {
      controller
        .getAllJoin("Users", username, "Recipe")
        .then(recipeData => {
  ...
  describe("Meals", () => {
    ...
    it("should retrieve the meal from the database", done => {
      controller.getAllJoin("Users", username, "Meals").then(mealData => {
  ...
  describe("Events", () => {
    ...
    it("should retrieve recipes from meals", done => {
      controller
        .getAllJoin("Meals", "Sausage and Chips", "Recipe")
        .then(data => {
          console.log(data);
          done();
        })
